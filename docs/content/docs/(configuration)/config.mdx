...
# ACP (Agent Client Protocol) coding workers. Delegates coding tasks to an
# ACP-compatible CLI (Claude Code, Codex, Cursor CLI) over JSON-RPC stdio.
[defaults.acp]
enabled = false                          # disabled until a CLI is configured
command = "claude"                       # ACP-compatible CLI to launch (supports env:VAR_NAME)
args = []                                # optional extra arguments
prompt_timeout_secs = 600                # per-turn prompt timeout
permissions = "auto_accept"              # "auto_accept" or "auto_reject"
